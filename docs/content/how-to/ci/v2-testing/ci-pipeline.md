...
Chains group related steps together. For Azure self-managed v2:

- **Create chain** (shared with v1): Provisions management cluster infrastructure, installs HyperShift operator
- **Run chain** (v2-specific): Executes the test matrix
- **Dump chain** (shared with v1): Collects must-gather artifacts
- **Destroy chain** (shared with v1): Tears down clusters and infrastructure

Only the workflow and run chain are v2-specific. The create, dump, and destroy chains are shared between v1 and v2 jobs.

### Refs

Refs are individual steps—thin shell scripts that invoke Go binaries from the `hypershift-tests` image. Here's the typical pattern:
...
with `E2E_HOSTED_CLUSTER_NAME` set to the appropriate cluster name.

**Parallel groups** run concurrently across multiple clusters. This is the default and maximizes throughput.

**Sequential groups** run steps one after another on the same cluster. If any step fails, remaining steps in that group are skipped. Use sequential groups for ordered workflows like upgrade → validate → downgrade.
...
Calls `hypershift dump cluster <platform>` in parallel for all clusters, collecting must-gather artifacts to `ARTIFACT_DIR`.
...
flowchart TD
    ...
    C -->|No| E["New ClusterSpec needed\n~15-20 min added to job"]
```
...
```diff
 func NewPlatformConfig(platform string) (PlatformConfig, error) {
     switch platform {
     case "azure":
         return NewAzureConfig(), nil
+    case "aws":
+        return NewAWSConfig(), nil
     default:
