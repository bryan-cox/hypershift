...
flowchart TD
    ...
    Image --> Binaries[/hypershift/bin/<br/>create-guests<br/>run-tests<br/>dump-guests<br/>destroy-guests]
...
- **TestContext** — Shared context initialized in `BeforeSuite` from environment variables. Provides management and hosted cluster clients, cluster name/namespace. Lazy-loads hosted cluster via `sync.Once`.
...
3. **create-guests** creates clusters in parallel — 5 phases: create, post-create hooks, wait Available, wait version rollout, write cluster names to `SHARED_DIR`
4. **run-tests** invokes `bin/test-e2e-v2` once per TestGroup with a different `--ginkgo.label-filter` and `E2E_HOSTED_CLUSTER_NAME`. Parallel groups run concurrently. Sequential groups run steps one after another on the same cluster.
