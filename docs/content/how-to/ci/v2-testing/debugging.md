...
Additionally, `create-guests` emits `junit_hosted_cluster_{name}.xml` when cluster creation or version rollout fails. These files contain the `HostedCluster` and `NodePool` conditions at the time of failure.
...
When a test is running, Ginkgo prints:

```text
[RUNNING] Control Plane Workloads Deployment generation kube-apiserver should not indicate rapid rollouts
```
...
The `dump-guests` binary **always exits 0**, so missing artifacts indicate a dump failure. To diagnose:
