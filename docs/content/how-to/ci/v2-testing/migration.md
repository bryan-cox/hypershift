...
In v1, each test owns its cluster lifecycle. `NewHypershiftTest.Execute()` creates a fresh cluster, runs the test function, then tears down the cluster. Tests are self-contained units that manage their own infrastructure:

```go
func TestMyFeature(t *testing.T) {
    ctx := context.Background()
    test := NewHypershiftTest(t, ctx, func(t *testing.T, g Gomega, mc crclient.Client, hc *hyperv1.HostedCluster) {
        // Test logic here - cluster is created, live, and will be destroyed after
    })
    test.Execute(&globalOpts, platform.None, t.Name())
}
```
...
| Test structure | `func TestFoo(t *testing.T)` + `NewHypershiftTest` | `Describe`/`It` blocks + `TestContext` |
| Subtests | `t.Run()` | Nested `It` blocks |
| Test selection | Separate binaries / build tags | Ginkgo labels + `--label-filter` |
| Cluster lifecycle | Per-test creation via `NewHypershiftTest.Execute()` | Pre-created by `create-guests`, shared via `TestContext` |
...
1. **Remove cluster lifecycle management** - Delete `NewHypershiftTest`, `Execute()`, and `globalOpts` calls. Your test no longer creates or destroys clusters.
...
3. **Add `BeforeEach` setup** - Acquire `TestContext`:
    ```go
    var testCtx *internal.TestContext
...
6. **Add appropriate labels** - Every `Describe` and `It` block should have labels for filtering. See [writing tests: labels](writing-tests.md#labels) for the label taxonomy.
