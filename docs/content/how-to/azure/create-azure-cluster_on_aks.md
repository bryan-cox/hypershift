...
```shell
AZURE_DISK_CLIENT_ID=$(az identity create --name azure-disk-dp-mi --resource-group $PERSISTENT_RG_NAME --query clientId -o tsv)
AZURE_FILE_CLIENT_ID=$(az identity create --name azure-file-dp-mi --resource-group $PERSISTENT_RG_NAME --query clientId -o tsv)
IMAGE_REGISTRY_CLIENT_ID=$(az identity create --name image-registry-dp-mi --resource-group $PERSISTENT_RG_NAME --query clientId -o tsv)
