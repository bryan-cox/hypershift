...
//   - Expect(target).To(HaveLen(...))    (any HaveLen implies non-empty)
//   - Expect(len(target)).To(BeNumerically(">", 0))
func extractNonEmptyGuardTarget(call *ast.CallExpr) string {
	...
	ast.Inspect(call, func(n ast.Node) bool {
		...
		case *ast.Ident:
			...
			if fn.Name == "BeNumerically" {
				hasBeNumerically = true
			}
	...
	// Expect(x).To(HaveLen(...))
	if hasPositive && hasHaveLen {
		return target
	}
	// Expect(len(x)).To(BeNumerically(">", 0))
	if hasPositive && hasBeNumerically {
		return target
	}
