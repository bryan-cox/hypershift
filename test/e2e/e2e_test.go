...
func TestMain(m *testing.M) {
	...
	flag.StringVar(&globalOpts.ConfigurableClusterOptions.AzureEncryptionKeyID, "e2e.encryption-key-id", "", "etcd encryption key identifier in the form of https://<vaultName>.vault.azure.net/keys/<keyName>/<keyVersion>")
	flag.StringVar(&globalOpts.ConfigurableClusterOptions.AzureKMSUserAssignedCredsSecretName, "e2e.kms-credentials-secret-name", "", "The name of a secret, in Azure KeyVault, containing the JSON UserAssignedIdentityCredentials used in KMS to authenticate to Azure.")
