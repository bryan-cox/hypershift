...
func adaptAzureWorkloadIdentityWebhookKubeconfigSecret(cpContext component.WorkloadContext, secret *corev1.Secret) error {
	...
	if !cpContext.SkipCertificateSigning {
		return pki.ReconcileServiceAccountKubeconfig(secret, csrSigner, rootCACM, cpContext.HCP, "openshift-cloud-credential-operator", "azure-workload-identity-webhook")
	}
	return nil
}
