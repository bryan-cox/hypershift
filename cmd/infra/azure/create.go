...
func createRhcosImages(ctx context.Context, l logr.Logger, o *CreateInfraOptions, subscriptionID string, resourceGroupName string, azureCreds azcore.TokenCredential) (string, error) {
	...
	imageBlobURL := "https://" + storageAccountName + ".blob.core.windows.net/" + "vhd" + "/"

	containerClient, err := container.NewClientWithSharedKeyCredential(imageBlobURL, credential, nil)
	if err != nil {
		...
		Properties: &armcompute.ImageProperties{
			StorageProfile: &armcompute.ImageStorageProfile{
				OSDisk: &armcompute.ImageOSDisk{
					...
					BlobURI: ptr.To(imageBlobURL + blobName),
				},
			},
...
func copyImageAndWait(ctx context.Context, rhcosURL string, pageBlobClient *pageblob.Client) error {
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("failed to check rhcos copy status: %w", err)
		}

		// This should never happen but just in case
		if properties.CopyStatus == nil {
			return fmt.Errorf("rhcos copy status is nil")
		}

		// Copy is complete; bail out of the timeout period
		if *properties.CopyStatus == blob.CopyStatusTypeSuccess {
			break
		}

		// Something went wrong with the copy process; bail out of the timeout period
		if *properties.CopyStatus == blob.CopyStatusTypeAborted || *properties.CopyStatus == blob.CopyStatusTypeFailed {
			return fmt.Errorf("failed to copy rhcos image: %w", err)
		}

		// The rhcos image did not copy within the timeout; bail out of the timeout period
		if time.Since(startTime) > timeout {
			return fmt.Errorf("failed to copy rhcos image within timeout period: %w", err)
		}

		// Poll every 5 seconds for copy status
